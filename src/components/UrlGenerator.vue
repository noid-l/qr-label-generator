<template>
  <section class="space-y-5">
    <article class="panel-card px-5 py-5 sm:px-6">
      <div class="mb-6">
        <h2 class="panel-title">URL 模式</h2>
        <p class="panel-subtitle">输入链接地址，扫码即可跳转。</p>
      </div>

      <form class="space-y-5" @submit.prevent>
        <div>
          <label class="field-label" for="url-input">链接地址</label>
          <input
            id="url-input"
            v-model="url"
            class="input-base"
        ...
        <div class="grid gap-5 sm:grid-cols-2">
          <div>
            <label class="field-label" for="url-size">二维码尺寸 (px)</label>
            <input
              id="url-size"
          ...
          <div>
            <label class="field-label" for="url-error">纠错等级</label>
            <select id="url-error" v-model="errorLevel" class="input-base">
              <option
...
const qrValue = computed(() => url.value.trim())
...
function handleDownload(): void {
  ...
  downloadQrImage(canvas, `qrcode_url_${Date.now()}.png`, qrSize.value)
  success('下载成功')
}
</script>
